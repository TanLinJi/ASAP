#!/usr/bin/env bash
# Run the adaptive-radius ASAP score-net purifier with M2 gating disabled.
#
# This isolates whether M1 adaptive SPU geometry alone is enough when every
# SPU is purified. It keeps the Track A score-net and anchor settings, but
# overrides tau to -1.0 so all SPUs pass the anomaly gate.
#
# Usage:
#   ATTACK=E2.2_perturbation bash scripts/run_adaptive_all_spu_score_net_dual_gpu.sh
#
# Optional env:
#   GPUS=0,1
#   WORKERS_PER_GPU=1
#   CKPT=checkpoints/kitti/asap_score_net_trackA_v1.pth
#   OUT_BASE=outputs/adaptive_all_spu_score_net
...
echo "[$(date +%H:%M:%S)] adaptive-all-SPU score-net attack=${ATTACK} frames=${i} gpus=${GPUS}"
echo "[$(date +%H:%M:%S)] tau=-1.0 score_anchor_blend=${SCORE_ANCHOR_BLEND}"

pids=()
for gpu in "${GPU_IDS[@]}"; do
    log="${LOGDIR}/adaptive_all_spu_${ATTACK}_gpu${gpu}.log"
